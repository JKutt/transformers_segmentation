...
def run_line_spacing_qc_agg(
    ...
    qc_execute: dict = {
        'line_seperation':True,
        'instrument_dropout':True
    }
    ...
    """
        ...
        :param qc_execute: run qc on instrument dropout
        :type qc_execute: bool
    ...
    csv_qc = open(output_file + '-line_spacing.csv', 'w+')
    csv_qc.write('line_id,easting,northing\n')

    csv_dropout_qc = open(output_file + '-time_dropout.csv', 'w+')
    csv_dropout_qc.write('line_id,easting1,northing1,easting2,northing2\n')

    # extract line ids
    line_ids = []

    # set the parameter for half the demodulation filter constant
    half_time_constant = (1/time_constant) / 2
    ...
    for ii in range(0, len(data_by_line)):
        try:
            ...
            if qc_execute['instrument_dropout']:
                ...
                if (difference_between_samples >= half_time_constant).sum() > 0:
                    ...
                    # grab the offending points
                    time_excess_points_end = samples[find_excess_time[0]]
                    time_excess_points_start = samples[find_excess_time[0] - 1]
                    time_excess_points = np.hstack([time_excess_points_end, time_excess_points_start])

                    # add points to time plot
                    ax.plot(time_excess_points[:, 0], time_excess_points[:, 1], '.r')
                    ax.annotate(line_ids[ii], xy=(time_excess_points[0, 0], time_excess_points[0, 1]))

                    for kk in range(time_excess_points_end.shape[0]):
                        csv_dropout_qc.write(f'{line_ids[ii]},{time_excess_points_start[kk, 0]},{time_excess_points_start[kk, 1]},{time_excess_points_end[kk, 0]},{time_excess_points_start[kk, 1]}\n')
                        if output_png:
                            ax.plot(time_excess_points_start[kk, 0], time_excess_points_start[kk, 1], '.g')
                            ax.plot(time_excess_points_end[kk, 0], time_excess_points_end[kk, 1], '.g')
                        ax.annotate(line_ids[ii], xy=(time_excess_points_start[0, 0], time_excess_points_start[0, 1]))
...
if __name__ == '__main__':
    ...
    if True:
        ...
        # data file to view
        agg_data = "/home/juan/Documents/projects/jcontract/j0002/data/2200168_agg_preliminary.csv"
        output_filepath = r'agg_preliminary_qc'

        # line seperation parameter
        line_spacing = 200

        # set the qc to execute in dictionary
        qc_check_dict = {
            'line_spacing': True,
            'instrument_dropout': True,
        }
